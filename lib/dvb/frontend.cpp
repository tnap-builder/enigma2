...
RESULT eDVBFrontendParameters::calcLockTimeout(unsigned int &timeout) const
{
	switch (m_type)
	{
		case iDVBFrontend::feSatellite:
		{
			...
			else if (sat.symbol_rate >= 2000000 && sat.symbol_rate <= 19999999)
			{
				eDebug("[eDVBFrontend#521] sat.symbol_rate = %d, timeout = 10000", sat.symbol_rate);
				timeout = 10000;
			}
			else if (sat.symbol_rate >= 800000 && sat.symbol_rate <= 1999999)
			{
				eDebug("[eDVBFrontend#521] sat.symbol_rate = %d, timeout = 10000", sat.symbol_rate);
				timeout = 30000;
			}
...
int eDVBFrontend::isCompatibleWith(ePtr<iDVBFrontendParameters> &feparm, bool is_configured_sat)
{
	...
	else if (type == eDVBFrontend::feTerrestrial)
	{
		...
		if (parm.system == eDVBFrontendParametersTerrestrial::System_DVB_T && can_handle_isdbt)
		{
			return 0;
		}
