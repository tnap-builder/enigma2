		...
		if csel.bouquet_mark_edit == OFF and not csel.entry_marked:
			if not inBouquetRootList:
				...
				if isPlayable:
					...
					if 'FROM SATELLITES' in current_root.getPath()  and not ("Current transponder") in eServiceCenter.getInstance().info(current).getName(current):
					...
					if not servicelist is None:
						while True:
							...
							if service_type:
								...
								if z ==1 and config.usage.show_New_folders.value == True:
									cntstr = " (%d)"%(self.getServicesCount(service))										
									service.setName("%s" % (service_name))																			    
									self.servicelist.addService(service)								    
										    
								if z == 1 and config.usage.show_New_folders.value == False:
									service.setName("%s " % (service_name))
									self.servicelist.addService(service)
								if z == 2 and config.usage.show_New_folders.value == True:
